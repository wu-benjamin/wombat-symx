...
pub fn is_panic_block(bb: &BasicBlock) -> Option<bool> {
    return if let Some(terminator) = bb.get_terminator() {
        ...
        match &opcode {
            ...
            _ => {
                warn!(
                    "Opcode {:?} is not supported as a terminator for panic detection",
                    opcode
                );
                None
            }
...
pub fn get_entry_condition<'a>(
    ...
    if let Some(terminator) =
        get_basic_block_by_name(function, &String::from(predecessor), namespace)
            .unwrap()
            .get_terminator()
    {
        let opcode = terminator.get_opcode();
        ...
        match &opcode {
            InstructionOpcode::Br => {
                if num_operands == 1 {
                    ...
                    warn!(
                        "Incorrect number of operators {:?} for opcode {:?} for edge generations",
                        num_operands, opcode
                    );
                }
            }
            InstructionOpcode::Switch => {
                ...
                for i in 0..num_operands {
                    if i % 2 == 1 {
                        ...
                        let basic_block_name =
                            format!("{}{}", namespace, basic_block.get_name().to_str().unwrap());
                ...
                if target_val == terminator.get_operand(0).unwrap().left().unwrap() {
                    ...
                    for j in 2..num_operands {
                        if j % 2 == 0 {
                            let temp_target_val =
                                terminator.get_operand(j).unwrap().left().unwrap();
                            let temp_target_val_var = Int::new_const(
                                solver.get_context(),
                                get_var_name(&temp_target_val, solver, namespace),
                            );
                            entry_condition = Bool::and(
                                solver.get_context(),
                                &[
                                    &(switch_var._eq(&temp_target_val_var)).not(),
                                    &entry_condition,
                                ],
                            );
                        }
                    ...
                    let target_val_var = Int::new_const(
                        solver.get_context(),
                        get_var_name(&target_val, solver, namespace),
                    );
                    entry_condition = switch_var._eq(&target_val_var);
                }
            }
...
pub fn codegen_basic_block(
    ...
    if let Some(successors) = forward_edges.get(&node) {
        for successor in successors {
            ...
            successor_conditions = Bool::and(
                solver.get_context(),
                &[&successor_conditions, &successor_var],
            );
        }
    }
    let mut node_var = successor_conditions;

    if forward_edges.get(&node).is_some()
        && forward_edges.get(&node).unwrap().contains(COMMON_END_NODE)
    {
        // assign panic_var
        let lvalue_var = Bool::new_const(solver.get_context(), PANIC_VAR_NAME);
        let is_panic =
            is_panic_block(&get_basic_block_by_name(function, &node, namespace).unwrap())
                .unwrap_or(true);
    ...
    let mut prev_instruction = get_basic_block_by_name(function, &node, namespace)
        .unwrap()
        .get_last_instruction();

    while let Some(current_instruction) = prev_instruction {
        // Process current instruction
        node_var = codegen_instruction(
            module,
            node_var,
            current_instruction,
            solver,
    ...
    if let Some(predecessors) = backward_edges.get(&node) {
        if !predecessors.is_empty() {
            for predecessor in predecessors {
                ...
                entry_conditions =
                    Bool::and(solver.get_context(), &[&entry_conditions, &entry_condition]);
            }
        }
    }
