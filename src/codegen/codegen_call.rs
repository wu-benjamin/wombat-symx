...
fn codegen_general_call<'a>(
    ...
    let called_function_forward_sorted_nodes =
        forward_topological_sort(&function, &new_namespace, post_node_name_str);
    ...
    assert!(
        u32::try_from(function.get_params().len()).unwrap() == instruction.get_num_operands() - 1
    );
    for i in 0..function.get_params().len() {
        let params = function.get_params();
        let input = params.get(i).unwrap();
        if input.get_type().to_string().eq("\"i1\"") {
            let lvalue = Bool::new_const(
                solver.get_context(),
                get_var_name(input, solver, &new_namespace),
            );
            let rvalue = Bool::new_const(
                solver.get_context(),
                get_var_name(
                    &instruction
                        .get_operand(u32::try_from(i).unwrap())
                        .unwrap()
                        .left()
                        .unwrap(),
                    solver,
...
pub fn codegen_call<'a>(
    ...
    let module_name =
        get_module_name_from_file_name(&String::from(module.get_name().to_str().unwrap()));
    ...
    match call_operation_name_str {
        s if s.starts_with("llvm.sadd.with.overflow.i") => {
            ...
            let rvalue_var_2 = Bool::or(
                solver.get_context(),
                &[&rvalue_var_1.gt(&max_int), &rvalue_var_1.lt(&min_int)],
            );

            let assignment_2 =
                Bool::new_const(solver.get_context(), lvalue_var_name_2)._eq(&rvalue_var_2);
        ...
        "llvm.expect.i1" => {
            ...
            let rvalue_var = Bool::new_const(solver.get_context(), operand1_name)
                ._eq(&Bool::new_const(solver.get_context(), operand2_name));
            let assignment =
                Bool::new_const(solver.get_context(), lvalue_var_name)._eq(&rvalue_var);
            node_var = assignment.implies(&node_var);
        }
