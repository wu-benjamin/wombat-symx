...
fn get_field_to_extract(instruction: &InstructionValue) -> String {
    ...
    String::from(
        &instruction_string
            [instruction_string.rfind(' ').unwrap() + 1..instruction_string.rfind('\"').unwrap()],
    )
}
    ...
    match &opcode {
        ...
        InstructionOpcode::Return => {
            if instruction.get_num_operands() == 0 {
                ...
                if operand.get_type().to_string().eq("\"i1\"") {
                    ...
                    warn!(
                        "Currently unsupported type {:?} for return {:?}",
                ...
                warn!(
                    "Currently unsupported number of operands {:?} for return {:?}",
        ...
        InstructionOpcode::Load => {
            ...
            if !instruction.get_type().is_int_type() {
                warn!(
                    "Currently unsupported type {:?} for load operand",
        ...
        InstructionOpcode::Store => {
            ...
            if !operand1.get_type().is_int_type() {
                warn!(
                    "Currently unsupported type {:?} for store operand",
                    operand1.get_type().to_string()
                )
            }
            let operand2 = instruction
                .get_operand(1)
                .unwrap()
                .left()
                .unwrap()
        ...
        InstructionOpcode::Xor => {
            let operand1_var_name = get_var_name(
                &instruction.get_operand(0).unwrap().left().unwrap(),
                solver,
                namespace,
            );
            let operand2_var_name = get_var_name(
            ...
            if !instruction.get_type().to_string().eq("\"i1\"") {
                warn!(
                    "Currently unsupported type {:?} for xor operand",
                    instruction.get_type().to_string()
                );
            }
        ...
        InstructionOpcode::ICmp => {
            let lvalue_var_name = get_var_name(&instruction, solver, namespace);
            let lvalue_var = Bool::new_const(solver.get_context(), lvalue_var_name);
            let operand1 = get_var_name(
                &instruction.get_operand(0).unwrap().left().unwrap(),
                solver,
                namespace,
            );
            let operand2 = get_var_name(
                &instruction.get_operand(1).unwrap().left().unwrap(),
                solver,
                namespace,
            );
            ...
            let rvalue_operation = match &icmp_type {
                IntPredicate::EQ => Int::new_const(solver.get_context(), operand1)
                    ._eq(&Int::new_const(solver.get_context(), operand2)),
                IntPredicate::NE => Int::new_const(solver.get_context(), operand1)
                    ._eq(&Int::new_const(solver.get_context(), operand2))
                    .not(),
                IntPredicate::SGE | IntPredicate::UGE => {
                    Int::new_const(solver.get_context(), operand1)
                        .ge(&Int::new_const(solver.get_context(), operand2))
                }
                IntPredicate::SGT | IntPredicate::UGT => {
                    Int::new_const(solver.get_context(), operand1)
                        .gt(&Int::new_const(solver.get_context(), operand2))
                }
                IntPredicate::SLE | IntPredicate::ULE => {
                    Int::new_const(solver.get_context(), operand1)
                        .le(&Int::new_const(solver.get_context(), operand2))
                }
                IntPredicate::SLT | IntPredicate::ULT => {
                    Int::new_const(solver.get_context(), operand1)
                        .lt(&Int::new_const(solver.get_context(), operand2))
                }
            };
        ...
        InstructionOpcode::ExtractValue => {
            let lvalue_var_name = get_var_name(&instruction, solver, namespace);
            let operand = instruction.get_operand(0).unwrap().left().unwrap();
            let rvalue_var_name = format!(
                "{}.{}",
                get_var_name(&operand, solver, namespace),
                get_field_to_extract(&instruction)
            );
            if instruction.get_type().to_string().eq("\"i1\"") {
                let lvalue_var = Bool::new_const(solver.get_context(), lvalue_var_name);
                ...
                warn!(
                    "Currently unsupported type {:?} for extract value",
                    operand.get_type().to_string()
                )
            }
        }
        ...
        InstructionOpcode::Trunc => {
            if instruction.get_type().to_string().eq("\"i1\"") {
                let lvalue_var_name = get_var_name(&instruction, solver, namespace);
                let operand_var_name = get_var_name(
                    &instruction.get_operand(0).unwrap().left().unwrap(),
                    solver,
                    namespace,
                );
                ...
                warn!(
                    "Type {} is not a supported target type for the Trunc instruction!",
                    instruction.get_type().to_string()
                );
            }
        }
        InstructionOpcode::Select => {
            ...
            let operand_1_var_name = get_var_name(
                &instruction.get_operand(1).unwrap().left().unwrap(),
                solver,
                namespace,
            );
            let operand_2_var_name = get_var_name(
                &instruction.get_operand(2).unwrap().left().unwrap(),
                solver,
                namespace,
            );
            if !discriminant.get_type().to_string().eq("\"i1\"") {
                warn!(
                    "Currently unsupported type {:?} for select discriminant",
                    discriminant.get_type().to_string()
                );
            }
            ...
            if instruction.get_type().to_string().eq("\"i1\"") {
                ...
                let select_1 = discriminant_var.implies(
                    &Bool::new_const(
                        solver.get_context(),
                        get_var_name(&instruction, solver, namespace),
                    )
                    ._eq(&operand_1_var),
                );
                let select_2 = discriminant_var.not().implies(
                    &Bool::new_const(
                        solver.get_context(),
                        get_var_name(&instruction, solver, namespace),
                    )
                ...
                node_var = Bool::and(
                    solver.get_context(),
                    &[&select_1.implies(&node_var), &select_2.implies(&node_var)],
                );
            } else if instruction.get_type().is_int_type() {
                ...
                warn!(
                    "Currently unsupported type {:?} for select",
                    instruction.get_type().to_string()
                );
            }
        }
        InstructionOpcode::ZExt => {
                ...
                .is_int_type()
            {
                ...
                warn!(
                    "Type {} is not a supported target type for the ZExt instruction!",
                    instruction.get_type().to_string()
                );
            }
        }
        _ => {
            warn!(
                "Opcode {:?} is not supported as a statement for code gen",
                opcode
            );
        }
    }
