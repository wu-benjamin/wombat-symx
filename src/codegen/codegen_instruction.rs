    ...
    match &opcode {
        ...
        InstructionOpcode::Select => {
            ...
            if instruction.get_type().to_string().eq("\"i1\"") {
                ...
                let select_1 =
                    discriminant_var.implies(&Bool::new_const(solver.get_context(), get_var_name(&instruction, solver, namespace))._eq(&operand_1_var));
                ...
                let select_1 =
                    discriminant_var.implies(&Int::new_const(solver.get_context(), get_var_name(&instruction, solver, namespace))._eq(&operand_1_var));
                let select_2 = discriminant_var
                    .not()
