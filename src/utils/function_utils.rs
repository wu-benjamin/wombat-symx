...
pub fn get_function_argument_names<'a>(
    function: FunctionValue<'a>,
    solver: &Solver,
    namespace: &str,
) -> Vec<(String, String, BasicTypeEnum<'a>)> {
        ...
        if param.get_name().is_empty() {
            ...
            while instr.is_some() {
                if instr.unwrap().get_opcode() == InstructionOpcode::Store
                    && *alias_name
                        == get_var_name(&instr.unwrap().as_any_value_enum(), solver, namespace)
                {
                    let arg_name = get_var_name(
                        &instr
                            .unwrap()
                            .get_operand(1)
                            .unwrap()
...
pub fn get_all_function_argument_names<'a>(
    ...
    while let Some(current_function) = next_function {
        let current_full_function_name =
            get_function_name(&current_function.as_global_value().as_pointer_value());
        let function_argument_names =
            get_function_argument_names(current_function, solver, namespace);
...
pub fn get_function_by_name<'a>(
    module: &'a InkwellModule,
    target_function_name_prefix: &String,
