        ...
        for cur_node in current_function.get_basic_blocks() {
            ...
            for instruction in instructions {
                if instruction.get_opcode() == InstructionOpcode::Phi {
                    ...
                    if current_function.get_first_basic_block().unwrap().get_first_instruction().is_some() {
                        builder.position_before(&current_function.get_first_basic_block().unwrap().get_first_instruction().unwrap());
                    }
                    let alloca = builder.build_alloca(phi_instruction_type, &format!("{}_ptr", var_name));
            ...
            for (phi_predecessor, assignments) in &phi_predecessor_assignments {
                ...
                if phi_predecessor_terminator.get_opcode() == InstructionOpcode::Br {
                    if phi_predecessor_terminator.get_num_operands() == 1 {
                        ...
                        let mut predecessor_dest_then_block = phi_predecessor_terminator.get_operand(2).unwrap().right().unwrap();
                        let mut predecessor_dest_else_block = phi_predecessor_terminator.get_operand(1).unwrap().right().unwrap();
                        if predecessor_dest_then_block == cur_node {
                            predecessor_dest_then_block = edge_node;
                        } else {
                            predecessor_dest_else_block = edge_node;
                        }
                        let condition = phi_predecessor_terminator.get_operand(0).unwrap().left().unwrap().into_int_value();
                        phi_predecessor_terminator.remove_from_basic_block();
                    ...
                    let predecessor_dest_else_block = phi_predecessor_terminator.get_operand(1).unwrap().right().unwrap();
                    let mut predecessor_dest_cases = Vec::<(IntValue, BasicBlock)>::new();
                    for i in 2..phi_predecessor_terminator.get_num_operands() {
                        if i % 2 == 0 {
                            let mut predecessor_dest_conditional_block = phi_predecessor_terminator.get_operand(i + 1).unwrap().right().unwrap();
                    ...
                    phi_predecessor_terminator.remove_from_basic_block();
                    builder.build_switch(predecessor_dest_else_value, predecessor_dest_else_block, &predecessor_dest_cases);
                } else {
